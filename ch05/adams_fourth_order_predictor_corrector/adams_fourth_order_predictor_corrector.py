...
def adams_fourth_order_predictor_corrector(
    ...
    while i <= 3 and (not isinstance(w[j], complex) for j in range(len(w))):
        try:
            ...
            k_2 = h * function(t[i-1] + h/2., w[i-1] + k_1/2.)
            k_3 = h * function(t[i-1] + h/2., w[i-1] + k_2/2.)
            k_4 = h * function(t[i-1] + h, w[i-1] + k_3)

            # STEP 4: Compute w_i, t_i
            w.append(w[i-1] + (k_1 + 2.*k_2 + 2.*k_3 + k_4)/6.)
            t.append(a + i*h)
    ...
    while i <= n and (not isinstance(w[j], complex) for j in range(len(w))):
        try:
            ...
            t.append(a + i*h)
            w.append(
                w[i-1] + h*(55*function(t[i-1], w[i-1]) - 59*function(t[i-2], w[i-2])
                           + 37*function(t[i-3], w[i-3]) - 9*function(t[i-4], w[i-4]))/24.
                           )
            w[i] = w[i-1] + h*(9*function(t[i], w[i]) + 19*function(t[i-1], w[i-1])
                           - 5*function(t[i-2], w[i-2]) + function(t[i-3], w[i-3]))/24.
